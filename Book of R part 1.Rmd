...
#(b) involves array which I did not cover (last section of chap3)
bar<-a[-(1:3)]
bar <- array(data=bar,dim=c(2,2,3)) #suggested solution
bar
bar<=(6/2+4) 
(bar+2)<=(6/2+4)
...
#(d) involving the 3d array in #b (use suggested solution for #b)
any(bar<=(6/2+4))
all(bar<=(6/2+4))
any((bar+2)<=(6/2+4))
all((bar+2)<=(6/2+4))

#(e) not sure about what the question is asking
any(diag(diag(10)==0))
```
...
bar_out
foo_out & bar_out
foo_bar<-foo_out & bar_out
#(d)
baz<-foo + bar
baz>=14 & baz!=15 ##i
baz_out <- baz>=14 & baz!=15 ##i store
baz/foo >4 | baz/foo<=2 ##ii
bazfoo_out<-baz/foo >4 | baz/foo<=2 ##ii store
#(e)
foofoo_out<- (foo>5) || (foo==2) ##a result long version
  foo_out[1]==foofoo_out ##a verify
barbar_out<- (bar<=6) && (bar!=4) ##b result long version
  bar_out[1]==barbar_out ##b verify
foo_out && bar_out ##c result long version
  foo_bar[1]==(foo_out && bar_out) ## c verify
#solution for c confirmation (used long version in all logical operators)
((foo>5)||(foo==2))&&((bar<=6)&&(bar!=4))
  
baz>=14 && baz!=15 ##d-i
  baz_out[1]==(baz>=14 && baz!=15) ##d-i verify
baz/foo >4 || baz/foo<=2 ##d-ii
  bazfoo_out[1]==(baz/foo >4 || baz/foo<=2) ##d-ii verify
```
...
bar<-foo[foo>=5] ##i
which(foo>=5) ##ii #returns the index positions in foo
foo[-which(foo>=5)] #omit foo >=5; returns the resulting values

#(b)
bar
baz<-matrix(bar, nrow=2, ncol=3, byrow=TRUE)
baz[1,2]^2
baz[baz==8] <- baz[1,2]^2 ##i
baz<=25 & baz>4 ##ii

#(c)
x<-c(10,5,1,4,7,4,3,3,1,3,4,3,1,7,8,3,7,3)
qux<-array(data=x, dim=c(3,2,3))
qux==3 | qux==4
```
